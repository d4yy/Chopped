...
import net.joey.chopped.blocks.crops.CropSectionProperty;
...
public class ChoppedBlockLootTables extends BlockLootSubProvider {
    ...
    protected void generate() {
        ...
        dropSelf(ChoppedBlocks.BLOCKS_HIMALAYAN_SALT.get());
        dropSelf(ChoppedBlocks.BLOCKS_SEA_SALT.get());
        ...
        add(ChoppedBlocks.BLOCKS_SEA_SALT.get(), block -> this.createSingleItemTableWithSilkTouch(block, ChoppedItems.ITEMS_SEA_SALT_PINCH.get(), ConstantValue.exactly(4.0F)));
    ...
    protected LootTable.Builder createMultiCropDrops(Block cropBlock, Item yieldItem, Item seedItem, Float minYield, Float maxYield, Float bonusAmount, Float bonusChance, Item... bonusItems) {
        LootItemCondition.Builder sufficientGrowthPropertyCondition = LootItemBlockStatePropertyCondition.hasBlockStateProperties(cropBlock).setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(MultiCropBlock.GROWN, true));
        ...
        LootTable.Builder table = LootTable.lootTable()
            .withPool(LootPool.lootPool()
                .add(LootItem.lootTableItem(yieldItem).apply(SetItemCountFunction.setCount(UniformGenerator.between(minYield, maxYield))).when(sufficientGrowthPropertyCondition).otherwise(LootItem.lootTableItem(seedItem).when(stalkBasePropertyCondition)))
            ).withPool(LootPool.lootPool().when(sufficientGrowthPropertyCondition)
                .add(LootItem.lootTableItem(seedItem).apply(ApplyBonusCount.addBonusBinomialDistributionCount(Enchantments.FORTUNE, 0.5714286F, 3)).when(stalkBasePropertyCondition))
            );
        ...
        if (bonusItems.length > 0) {
            for (Item bonusItem : bonusItems) {
                table.withPool(LootPool.lootPool().add(LootItem.lootTableItem(bonusItem)).setRolls(ConstantValue.exactly(1.0F))
                    .when(sufficientGrowthPropertyCondition)
    ...
    protected Iterable<Block> getKnownBlocks() {
        return ChoppedRegistry.BLOCKS.getEntries().stream().map(Holder::value).collect(Collectors.toList());
    }
}
